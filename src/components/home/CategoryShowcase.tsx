        ...
        {categories.slice(0, 6).map((c, i) => (
          <Link key={c.slug} href={'/kategori/' + c.slug} className="group min-w-0">
            <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-[#eee9e1]">
              ...
              <span className="absolute left-3 top-3 text-[11px] font-semibold text-ink">0{i + 1}</span>
              <span className="absolute bottom-3 right-3 grid h-8 w-8 place-items-center rounded-full bg-cream/95 text-ink transition-colors group-hover:bg-brand-500 group-hover:text-white"><ArrowUpRight size={16} /></span>
            </div>
