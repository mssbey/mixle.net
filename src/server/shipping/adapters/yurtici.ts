// Yurtiçi Kargo adaptörü — İSKELET.
//
// Yurtiçi Kargo'nun sevkiyat oluşturma/etiket/takip API'si bayi/entegrasyon
...
export async function yurticiProvider(): Promise<ShippingProvider> {
  const settings = await getShippingSettings();
  const cfg = settings.providers.yurtici;
  ...
  return {
    ...
    async createShipment() {
      throw new ShippingProviderError(
        'Yurtiçi Kargo için gerçek API entegrasyonu bu sürümde yok. Takip numarasını panelden elle girin.',
        501,
      );
    },
    async track() {
      throw new ShippingProviderError('Yurtiçi Kargo için otomatik takip henüz bağlı değil.', 501);
    },
    async cancel() {
      throw new ShippingProviderError('Yurtiçi Kargo için otomatik iptal henüz bağlı değil.', 501);
    },
  };
}
